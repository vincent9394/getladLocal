...
searchRoute.get('/search/type/:type' ,async (req,res)=>{
    // /search/type/abc

    const notes = await client.query(/*sql*/`
        WITH most_joined_events AS (
            SELECT 
            id, 
            ( SELECT count(*) 
            FROM join_group 
        ...
        where 
            event_types.type = $2
        ORDER BY events.created_at ASC;`, [req.session['user'],req.params.type])
    res.json(notes.rows);
});
